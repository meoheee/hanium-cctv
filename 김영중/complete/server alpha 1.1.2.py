...
while 1:
    ...
    conn, addr = s.accept()
    print('연결')
    savevideo = cv2.VideoWriter('vid.avi', cv2.VideoWriter_fourcc(*'XVID'), 15.0, (1280, 720))
    while 1 :
        ...
        if i % 8 == 0:
            ...
            for face in detections:
                l, t, r, b = (face.rect.left(), face.rect.top(), face.rect.right(), face.rect.bottom())
                points = points_detector(decimg, face.rect)
                face_descriptor = face_descriptor_extrator.compute_face_descriptor(decimg, points)
        ...
        if detections:
            ...
            cv2.putText(decimg, f'{name},{min_distance}', (face.rect.left(), face.rect.top()),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0))
